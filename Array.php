<?php
    // Array of programs with names and corresponding PHP file paths
    $programs = [
        '1. PHP Arrays, Creating Arrays, Accessing Array elements in PHP' => './Program_Folder/Program23.php',
        '2. Explaining Multidimensional Arrays in PHP' => './Program_Folder/Program24.php',
        '3. Inspecting and Deleting from Arrays in PHP' => './Program_Folder/Program25.php',
        '4. Iterative functions in PHP' => './Program_Folder/Program26.php',
    ];
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Array Programs</title>
    <style>
        .List {
            font-size: 20px;
            color: black;
            text-decoration: none;
            margin-top: 15px;
        }
        .List:hover {
            color: blue;
        }
    </style>
</head>
<body>
    <?php foreach ($programs as $name => $link): ?>
        <a class="List" href="javascript:void(0)" onclick="loadProgramAndOutput('<?php echo $link; ?>')">
            <?php echo $name; ?>
        </a><br />
    <?php endforeach; ?>

    <script>
        function loadProgramAndOutput(programSrc) {
            // Load the selected program into the "Program" iframe
            parent.frames["Program"].location.href = programSrc;
        }
    </script>
</body>
</html>
